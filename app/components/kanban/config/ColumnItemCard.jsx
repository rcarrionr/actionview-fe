...
@DropTarget(CardTypes.KANBAN_COLUMN, cardTarget, connect => ({
...
@DragSource(
  props => {
    if (!props.isAllowedEdit) {
      ...
      return CardTypes.KANBAN_COLUMN;
    }
  },
  cardSource, 
  (connect, monitor) => ({
    ...
    return connectDragSource(connectDropTarget(
      <div style={ { opacity } } className='config-column'>
        <div style={ { fontWeight: 600, paddingBottom: '10px', borderBottom: '1px solid #ccc' } }>
          ...
          { isAllowedEdit && !!editColumn && <span style={ styles } onClick={ () => { editColumn(column.no) } } title='edit' className='edit-icon'><i className='fa fa-edit'></i></span> }
        </div>
        <div>
