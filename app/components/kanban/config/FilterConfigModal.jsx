    ...
    return (
      <Modal 
        ...
        <Modal.Header closeButton>
          <Modal.Title id='contained-modal-title-la'>{ model == 'global' ? 'Global filter' : ( no === -1 ? 'Add fast filter' : 'Edit fast filter' ) }</Modal.Title>
        </Modal.Header>
        ...
        <Modal.Body style={ { maxHeight: '580px', overflow: 'auto', paddingBottom: '0px' } }>
          ...
          <FormGroup 
            ...
            <Col sm={ 2 } componentClass={ ControlLabel }>
              <span className='txt-impt'>*</span>Filter name 
            </Col>
            <Col sm={ 8 }>
              <FormControl
                type='text'
                value={ this.state.name }
                onChange={ (e) => { this.setState({ name: e.target.value }); if (!e.target.value) { this.state.errors.name = 'Be required'; this.setState({ errors: this.state.errors }); } else { this.setState({ errors: {} }); } } }
        ...
        <Modal.Footer>
          ...
          <Button disabled={ (model === 'filter' && (!this.state.name || _.isEmpty(this.state.query))) || loading } onClick={ this.handleSubmit }>Sure</Button>
