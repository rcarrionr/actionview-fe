...
export default class CopyModal extends Component {
  ...
  async searchPath(input) {
    ...
    const limit = 20;
    const results = await api.request( { url: '/project/' + project_key + '/document/search/path?s=' + input + ( data.d === 1 ? ('&moved_path=' + data.id) : '' ) } );
    ...
    return (
      <Modal show bsSize='large' onHide={ this.handleCancel } onEntered={ this.handleEntry } backdrop='static' aria-labelledby='contained-modal-title-sm'>
        <Modal.Header closeButton>
          <Modal.Title id='contained-modal-title-la'>move{ data.d === 1 ? 'content' : 'Documentation' } - { data.name }</Modal.Title>
        </Modal.Header>
