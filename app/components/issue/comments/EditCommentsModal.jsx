  ...
  async confirm() {
    ...
    if (data.parent_id) {
      ...
      if (ecode === 0) {
        close();
        if (data.id) {
          notify.show('Reply is updated.', 'success', 2000);
        } else {
          notify.show('Reply has been added.', 'success', 2000);
        }
      }
    } else {
      ...
      if (ecode === 0) {
        close();
        notify.show('Updated comments.', 'success', 2000);
      }
    }
  }
    ...
    return (
      <Modal show onHide={ this.cancel } backdrop='static' aria-labelledby='contained-modal-title-sm'>
        ...
        <Modal.Footer>
          ...
          <Button disabled={ this.state.oldContents === this.state.contents || loading } onClick={ this.confirm }>Sure</Button>
