...
export default class Comments extends Component {
  ...
  async addComments() {
    ...
    if (ecode === 0) {
      ...
      notify.show('Comments have been added.', 'success', 2000);
    } else {
      notify.show('Comment fails.', 'error', 2000);
    }
  }
        ...
        <FormGroup>
          <Col sm={ 12 } className={ indexLoading && 'hide' } style={ { marginTop: '15px', marginBottom: '15px' } }>
            <div>
              ...
              <span className='comments-button' title='Add to' style={ { marginRight: '10px', float: 'right' } } onClick={ this.showCommentsInputor.bind(this) }><i className='fa fa-comment-o'></i> Add to</span> }
          ...
          <Col sm={ 12 } className={ this.state.addCommentsShow || 'hide' }>
            <div className='comments-inputor'>
              <FormControl
                ...
                placeholder='support@project members,Ctrl+EnterRelease comments.' />
            </div>
            <div style={ { textAlign: 'right', marginBottom: '10px' } }>
              <img src={ img } className={ loading ? 'loading' : 'hide' } />
              <Button style={ { marginLeft: '10px' } } onClick={ this.addComments } disabled={ loading || _.isEmpty(_.trim(this.state.contents)) }>Add to</Button>
          ...
          { collection.length <= 0 && !indexLoading ?
            <div style={ { width: '100%', textAlign: 'left', marginTop: '10px', marginLeft: '10px' } }>no comment yet.</div>
            :
            _.map(collection, (val, i) => {
              const header = ( <div style={ { fontSize: '12px' } }>
                <span dangerouslySetInnerHTML= { { __html: '<a title="' + (val.creator && (val.creator.name + '(' + val.creator.email + ')')) + '">' + (val.creator && val.creator.id === currentUser.id ? 'I' : val.creator.name) + '</a> - ' + (this.state.displayTimeFormat == 'absolute' ? moment.unix(val.created_at).format('YYYY/MM/DD HH:mm:ss') : getAgoAt(val.created_at, currentTime)) + (val.edited_flag == 1 ? '<span style="color:red"> - Edited</span>' : '') } } />
                ...
                <span className='comments-button comments-edit-button' style={ { marginLeft: '7px', float: 'right' } } onClick={ this.showEditComments.bind(this, val) } title='edit'><i className='fa fa-edit'></i></span> }
                { permissions.indexOf('add_comments') !== -1 && 
                <span className='comments-button comments-edit-button' style={ { marginLeft: '7px', float: 'right' } } onClick={ this.showAddReply.bind(this, val.id, {}) } title='Reply'><i className='fa fa-reply'></i></span> }
              </div> ); 
                     ...
                     { _.map(val.reply, (v, i) => {
                       ...
                       <li className='reply-contents-item'>
                         <div className='reply-item-header'>
                           <span dangerouslySetInnerHTML= { { __html: '<a title="' + (v.creator && (v.creator.name + '(' + v.creator.email + ')')) + '">' + (v.creator && v.creator.id === currentUser.id ? 'I' : v.creator.name) + '</a> - ' + (this.state.displayTimeFormat == 'absolute' ? moment.unix(v.created_at).format('YYYY/MM/DD HH:mm:ss') : getAgoAt(v.created_at, currentTime)) + (v.edited_flag == 1 ? '<span style="color:red"> - Edited</span>' : '') } }/>
                           ...
                           { ((v.creator && currentUser.id === v.creator.id && permissions.indexOf('edit_self_comments') !== -1) 
                             || permissions.indexOf('edit_comments') !== -1) && 
                           <span className='comments-button comments-edit-button' style={ { marginLeft: '7px', float: 'right' } } onClick={ this.showEditReply.bind(this, val.id, v) } title='edit'><i className='fa fa-edit'></i></span> }
                           { permissions.indexOf('add_comments') !== -1 && 
                           <span className='comments-button comments-edit-button' style={ { marginLeft: '7px', float: 'right' } } onClick={ this.showAddReply.bind(this, val.id, v.creator) } title='Reply'><i className='fa fa-reply'></i></span> }
                         </div>
                         <div 
                           onClick={ this.previewInlineImg }
                           style={ { lineHeight: '24px', whiteSpace: 'pre-wrap', wordWrap: 'break-word' } } 
