        ...
        <FormGroup>
          <Col sm={ 12 } className={ indexLoading && 'hide' } style={ { marginTop: '15px', marginBottom: '15px' } }>
            <div>
              ...
              <span className='comments-button' title='Add to' style={ { marginRight: '10px', float: 'right' } } disabled={ loading } onClick={ this.showAddWorklog.bind(this) }><i className='fa fa-plus'></i> Add to</span> }
          ...
          { collection.length <= 0 && !indexLoading ?
            <div style={ { width: '100%', textAlign: 'left', marginTop: '10px', marginLeft: '10px' } }>No work records.</div>
            :
            _.map(collection, (val, i) => {
              const header = ( <div style={ { fontSize: '12px' } }>
                <span dangerouslySetInnerHTML={ { __html: '<a title="' + (val.recorder && (val.recorder.name + '(' + val.recorder.email + ')')) + '">' + (val.recorder.id === currentUser.id ? 'I' : val.recorder.name) + '</a> - ' + (this.state.displayTimeFormat == 'absolute' ? moment.unix(val.recorded_at).format('YYYY/MM/DD HH:mm:ss') : getAgoAt(val.recorded_at, currentTime)) + (val.edited_flag == 1 ? '<span style="color:red"> - Edited</span>' : '') } } />
                ...
                <span className='comments-button comments-edit-button' style={ { marginLeft: '7px', float: 'right' } } onClick={ this.showEditWorklog.bind(this, val) }><i className='fa fa-edit' title='edit'></i></span> }
              </div> ); 
              ...
              return (
                <Panel header={ header } key={ i } style={ { marginBottom: '15px' } }>
                  ...
                  <div style={ { marginLeft: '5px', lineHeight: '24px' } }>
                    <span style={ { width: '10%', float: 'left', fontWeight: 'bold' } }>Remark:</span>
